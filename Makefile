...
prepare::
	...
	if [[ "${OS}" != "Darwin" ]]; then \
		sed -i 's,github.com/blandman/pulumi-mongodbatlas,${REPOSITORY},g' go.mod; \
...
build:: tfgen provider
	for LANGUAGE in "nodejs" "python" "go" ; do \
		go run cmd/$(TFGEN)/main.go $$LANGUAGE --overlays overlays/$$LANGUAGE/ --out ${PACKDIR}/$$LANGUAGE/ || exit 3 ; \
	done
	cd ${PACKDIR}/nodejs/ && \
		yarn install && \
		yarn run tsc && \
...
tfgen::
	go install -ldflags "-X github.com/blandman/pulumi-${PACK}/pkg/version.Version=${VERSION}" ${PROJECT}/cmd/${TFGEN}

provider::
	go install -ldflags "-X github.com/blandman/pulumi-${PACK}/pkg/version.Version=${VERSION}" ${PROJECT}/cmd/${PROVIDER}

lint::
	golangci-lint run

install::
	GOBIN=$(PULUMI_BIN) go install -ldflags "-X github.com/blandman/pulumi-${PACK}/pkg/version.Version=${VERSION}" ${PROJECT}/cmd/${PROVIDER}
